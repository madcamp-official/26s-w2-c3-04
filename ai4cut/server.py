...
(이 폴더에 .env 파일을 두면 서버 시작 시 자동으로 읽어 환경변수로 등록합니다.
 별도 패키지 설치 없이 표준 라이브러리만으로 처리합니다.)
...
def load_dotenv(path=".env"):
    """.env 파일(KEY=VALUE 줄 단위)을 읽어 아직 설정되지 않은 환경변수만 채운다.
    실제 셸에서 export한 값이 있으면 그쪽이 우선한다."""
...
if __name__ == "__main__":
    load_dotenv()
    if not os.environ.get("CF_ACCOUNT_ID") or not os.environ.get("CF_API_TOKEN"):
        print("경고: CF_ACCOUNT_ID / CF_API_TOKEN이 설정되지 않았습니다. "
              "이 폴더에 .env 파일을 만들거나 환경변수로 export 하세요. "
              "지금 상태로는 프레임 생성 요청이 전부 실패합니다.", file=sys.stderr)
