...
- **산출물 주제:** AI 인생네컷 — 손동작(포즈) 인식으로 자동 촬영하고, AI가 테마에 맞는 프레임을 생성해주는 웹 네컷 사진 부스
- **제작 목적:** 사진관에 가지 않아도 웹캠만으로 인생네컷을 찍을 수 있게 하고, 정해진 프레임 대신 사용자가 원하는 테마를 AI(LLM + 이미지 생성)로 매번 새롭게 만들어내는 경험을 제공
- **선택 옵션:** LLM Wrapper
- **핵심 구현 요소:**
  ...
  - 4컷 합성·다운로드 — 촬영한 사진 중 4장을 선택해 생성된 프레임과 브라우저 캔버스에서 합성
- **사용 / 시연 시나리오:**
  1. 웹 페이지 접속 후 카메라 권한 허용
  2. 화면에 안내된 포즈를 취하면 인식 → 카운트다운 → 자동 촬영 (반복 촬영)
  3. 찍힌 사진 중 마음에 드는 4장 선택
  4. 원하는 프레임 테마를 자유 텍스트로 입력 (예: "숲속에서 찍은 느낌") → AI가 프레임 생성
...
| Day 3 | 손동작 인식 프로토타입 + AI 네컷 프로토타입 제작 |
| Day 4 | AI 프레임 생성(인페인팅) 프롬프트·품질 개선 |
| Day 5 | 포즈 인식·네컷 촬영·프레임 생성 통합, Express 백엔드 분리 |
| Day 6 | AWS App Runner 배포, 손동작 인식 정확도 개선 |
| Day 7 |  |
...
| 구현 요소 | 설명 | 우선순위 |
|---|---|---|
| 손동작 인식 자동 촬영 | MediaPipe HandLandmarker로 손 랜드마크를 추적하고, 포즈 판정 시 카운트다운 후 자동 촬영 | 필수 |
| AI 프레임 생성 | Cloudflare Workers AI 인페인팅 모델로 테마에 맞는 네컷 프레임 이미지 생성 | 필수 |
| 4컷 합성·다운로드 | 촬영 사진 4장을 생성된 프레임과 캔버스에서 합성해 다운로드 | 필수 |
| 자유 테마 키워드 변환 | 사용자가 입력한 한국어 자유 테마를 LLM이 영어 모티프 키워드로 변환 | 선택 |
| AWS 배포 | Express 백엔드를 AWS App Runner에 배포, Cloudflare 인증값은 Secrets Manager로 관리 | 선택 |
...
브라우저 (public_index.html)
  ├─ MediaPipe HandLandmarker 손동작 인식 (온디바이스, WASM)
  ├─ 웹캠 촬영 · 사진 선택
  ├─ 프레임 베이스 이미지 · 마스크 생성
