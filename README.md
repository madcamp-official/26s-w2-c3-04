...
- **사용 / 시연 시나리오:**
  ...
  2. 화면에 안내된 포즈를 취하면 인식 → 카운트다운 → 자동 촬영
  3. 찍힌 사진 중 마음에 드는 4장 선택
  4. 원하는 프레임 테마 선택 or  원하는 스타일 자유 텍스트로 입력 (예: "숲속에서 찍은 느낌") or 참고 이미지 업로드 or 대학교 프레임 선택 → AI가 프레임 생성
  5. 4컷 합성 결과 확인 후 이미지 다운로드
- **팀원별 역할:**
  - 정유진: 손동작 인식 프로토타입 구현, 프론트·백엔드 통합, AWS EC2 배포
...
| Day 6 | AWS EC2 배포, UI 개선, 프레임 생성 프롬프트 개선, 손동작 인식 개선 완료|
| Day 7 | 최종 완성 |
...
| AWS 배포 | Express 백엔드를 AWS EC2에 배포, Cloudflare 인증값은 Secrets Manager로 관리 |
...
> 🔗 **배포 링크:** [_(StoryBooth)_](https://3-39-22-65.sslip.io/univ_ver3.html)
...
- AI를 통해 모션 인식 로직을 구현해 볼 수 있어서 좋았다. 처음 해보는 과정이었지만 AI가 생각보다 정확하게 손동작을 인식하는 것 같아 신기하고 뿌듯한 경험이었다.
- 프레임이 시중 네컷 프레임처럼 나오지 않고 처음에는 동화책 일러스트 처럼 나오다가, 지금은 프롬프트를 입력하면 현실적인 사진처럼 나오는데, AI를 통해 미감을 개선하는 것이 생각보다 어렵다는 점을 알게되었다.
